...
app.post('/login', async (req, res) => {
  ...
  // const userinfo = User.find({ username})
  var flag = false
  result.forEach(item => {
...
app.post('/edit', async (req, res) => {
  // 查找旧名
  const result1 = await User.find({ username: req.body.oldusername })
  // 查找新名
  const result2 = await User.find({ username: req.body.editInfo.username })
  console.log(req.body.oldusername,req.body.editInfo.username)
  if(req.body.oldusername !==req.body.editInfo.username&&result2.length>0){
    res.json({
      code: 20001,
      msg: '用户名重复，请重试',
    })
  }else{
    User.findByIdAndUpdate(req.body._id, req.body.editInfo).then(result => {
      res.json({
        code: 20000,
        msg: '修改成功'
      })
...
app.get('/aaaaa',(req,res)=>{
  res.json({
    code:200,
    msg:"asdasdasdasddddddddddddddddddd"
  })
})
app.listen(8000)
